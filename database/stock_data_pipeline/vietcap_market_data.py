...
def build_market_data_record(
    ...
    is_today = bar["trade_date"] == date.today().isoformat()
...
def crawl_market_data(
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(crawl_one, s): s for s in symbols}
    ...
    log.info("Hoàn tất: %d thành công, %d rỗng, %d lỗi.", success, empty, failed)
...
def cmd_history(args):
    listing_client = VietcapMarketClient()

    if args.all:
        symbols = [s["symbol"] for s in listing_client.get_all_symbols()]
        log.info("Sẽ crawl lịch sử cho %d mã (toàn thị trường).", len(symbols))
    else:
