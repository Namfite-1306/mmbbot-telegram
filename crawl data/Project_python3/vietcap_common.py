...
def _ordered_fieldnames(rows: Iterable[dict]) -> List[str]:
    """Return the union of row keys while preserving first-seen order."""
    fieldnames: List[str] = []
    seen = set()
    for row in rows:
        for key in row:
...
def merge_csv_dedup(
    new_rows: List[dict],
    path: Path,
    key_field: str = "trade_date",
    key_fields: Optional[Sequence[str]] = None,
):
    """
    Gộp new_rows vào file CSV đã có và loại trùng theo một khóa hoặc
    khóa ghép. ``key_field`` được giữ để tương thích code cũ; code mới
    nên dùng ``key_fields=(...)`` khi một cột không đủ xác định bản ghi.
    """
    if not new_rows:
        return 0
    ...
    keys = tuple(key_fields or (key_field,))

    def make_key(row: dict):
        return tuple(row.get(field) for field in keys)

    merged = {make_key(row): row for row in existing_rows}
    ...
    all_rows = sorted(
        merged.values(),
        key=lambda row: tuple(str(row.get(field) or "") for field in keys),
    )
